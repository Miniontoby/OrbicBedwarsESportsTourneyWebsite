...
def get_player_data(username: str):
    ...
    if result['success']:
        ...
        displayname = result['player']['displayname'] = result['player']['displayname'] or (result['player']['knownAliases'] and (result['player']['knownAliases'][len(result['player']['knownAliases']) - 1])) or result['player']['playername'] or result['player']['username']
        rank = result['player']['rank'] = get_player_rank(result['player'])
        bedwarsLevel = result['player']['bedwarsLevel'] = get_bedwars_level_info(result['player'])
        print(displayname, '| Rank: ' + rank['cleanName'], '| BW Level: ' + str(bedwarsLevel['level']) + ' stars');
